...
export default function App() {
  ...
  const [sel, setSel] = useState<PipelineItem | null>(null);
  ...
  useEffect(() => {
    ...
    setError('');
    Promise.all([
      api.getItems(projectId, stage),
      api.getFeedback(projectId),
      api.getContradictions(projectId),
    ]).then(([itemsRes, fbRes, contraRes]) => {
  ...
  const selItem = useMemo(() => {
    if (!sel) return null;
    return items.find(i => i.id === sel.id) || sel;
  }, [sel, items]);
  ...
  const refreshItems = useCallback(async () => {
    if (!projectId) return;
    const res = await api.getItems(projectId, stage);
    setItems(res.items || []);
    const fbRes = await api.getFeedback(projectId);
    setFeedback(fbRes.feedback || []);
    const cRes = await api.getContradictions(projectId);
  ...
  const doStatus = useCallback(async (itemId: string, status: Status, cmt?: string) => {
    if (!projectId) return;
    try {
      await api.updateStatus(projectId, stage, itemId, status, cmt || undefined);
      await refreshItems();
      setComment('');
    } catch (e: any) { setError(e.message); }
  }, [projectId, stage, refreshItems]);

  const doComment = useCallback(async (itemId: string, text: string) => {
    if (!projectId || !text.trim()) return;
    try {
      await api.addComment(projectId, stage, itemId, text);
      await refreshItems();
      setComment('');
    } catch (e: any) { setError(e.message); }
  }, [projectId, stage, refreshItems]);
  ...
  const doGenerate = useCallback(async (direction?: string) => {
    ...
    try {
      if (stage === 'strategies') {
        await api.generateStrategies(projectId, 5, direction);
      } else if (stage === 'concepts') {
        const approvedStrategies = items.filter(i => i.status === 'approved');
        const stratId = approvedStrategies.length > 0 ? approvedStrategies[0].id : undefined;
        await api.generateConcepts(projectId, 5, stratId, direction);
      } else if (stage === 'scripts') {
        await api.generateScriptsBatch(projectId);
      } else if (stage === 'storyboards') {
        await api.generateStoryboardBatch(projectId);
      }
      await refreshItems();
    ...
    setGenerating(false);
  }, [projectId, stage, items, refreshItems]);

  const doIterate = useCallback(async (itemId: string, direction?: string) => {
    if (!projectId) return;
    setGenerating(true);
    try {
      await api.iterate(projectId, stage, itemId, direction);
      await refreshItems();
    } catch (e: any) { setError(e.message); }
    setGenerating(false);
  }, [projectId, stage, refreshItems]);
  ...
  const doAnalyze = useCallback(async () => {
    ...
    setGenerating(true);
    try {
      const res = await api.analyzeFeedback(projectId);
  ...
  const stageConf: Record<Stage, { label: string; icon: string; desc: string }> = {
    strategies: { label: 'Strategy', icon: '📊', desc: 'Approve strategic directions' },
    concepts: { label: 'Concepts', icon: '💡', desc: 'Review creative concepts' },
    scripts: { label: 'Scripts', icon: '📝', desc: 'Finalize shot lists' },
    storyboards: { label: 'Storyboard', icon: '🎬', desc: 'Visual frame review' },
  };
      ...
      <header className="flex-shrink-0" style={{ borderBottom: '1px solid #1a1a22', background: 'rgba(15,15,20,0.5)' }}>
        <div className="flex items-center justify-between px-5 py-3">
          <div className="flex items-center gap-3">
            <button onClick={() => { setProjectId(null); setSel(null); }} className="hover:opacity-80">
        ...
        <div className="px-5 pb-3 flex items-center gap-1">
          {(['strategies', 'concepts', 'scripts', 'storyboards'] as Stage[]).map((s, i) => (
            <div key={s} className="flex items-center flex-1">
              <button
                onClick={() => { setStage(s); setSel(null); setFilter('all'); }}
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="p-3 space-y-1.5">
                ...
                {filtered.map(item => (
                  <button key={item.id} onClick={() => { setSel(item); setComment(''); }}
                    className="w-full text-left rounded-lg p-3 transition-all animate-fade-in"
                    style={{
                      background: selItem?.id === item.id ? 'rgba(57,255,20,0.04)' : 'transparent',
                      border: selItem?.id === item.id ? '1px solid rgba(57,255,20,0.25)' : '1px solid transparent',
                    }}>
                    ...
                    {(item.comments?.length || 0) > 0 && (
                      <div className="mt-1.5 text-[10px]" style={{ color: '#555' }}>💬 {item.comments.length}</div>
                    )}
                  </button>
                ))}
              </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {selItem.status === 'revision' && (
                        <button onClick={() => doIterate(selItem.id, comment || undefined)} disabled={generating}
                          ...
                          {generating ? <span className="spinner" /> : '🔄 AI Iterate'}
                        </button>
                      )}
                    </div>
                  </div>
                        ...
                        {selItem.script.map((shot, i) => (
                          <div key={i} className="flex gap-3 p-3 rounded-md" style={{ background: '#111115', border: '1px solid #1a1a22' }}>
                            ...
                            <div className="flex-1">
                              ...
                              {shot.camera && <p className="text-[10px] mt-1" style={{ color: '#555' }}>📷 {shot.camera}</p>}
                              {shot.audio && <p className="text-[10px]" style={{ color: '#555' }}>🔊 {shot.audio}</p>}
                              {shot.text_overlay && <p className="text-[10px]" style={{ color: '#B8A9C9' }}>📝 "{shot.text_overlay}"</p>}
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="space-y-2">
                        {selItem.comments.map(c => (
                          <div key={c.id} className="flex gap-2 p-2.5 rounded-md"
                            ...
                            <span className="text-xs mt-0.5">{c.author === 'user' ? '👤' : '🤖'}</span>
                            <div className="flex-1">
                  ...
                  <div className="flex gap-2">
                    {comment.trim() && (
                      <button onClick={() => selItem && doComment(selItem.id, comment)}
                        className="text-xs px-3 py-1.5 rounded-md" style={{ color: '#888' }}>💬 Comment only</button>
                    )}
                  </div>
                </div>
              </div>
      ...
      {showMem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={() => setShowMem(false)}>
          <div className="rounded-lg w-full max-w-lg max-h-[80vh] flex flex-col" style={{ background: '#111115', border: '1px solid #1a1a22' }}
            ...
            <div className="p-4" style={{ borderBottom: '1px solid #1a1a22' }}>
              <h2 className="font-semibold flex items-center gap-2" style={{ color: '#e5e5e5' }}>🧠 Decision Memory</h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              {[...feedback].reverse().map(e => (
                <div key={e.id} className="p-3 rounded-md" style={{ border: '1px solid rgba(26,26,34,0.5)', background: 'rgba(17,17,21,0.3)' }}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs">{e.action === 'approved' ? '✅' : e.action === 'rejected' ? '❌' : e.action === 'revision' ? '🔄' : '💬'}</span>
      ...
      {showAnalysis && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={() => setShowAnalysis(false)}>
          <div className="rounded-lg w-full max-w-2xl max-h-[80vh] flex flex-col" style={{ background: '#111115', border: '1px solid rgba(184,169,201,0.3)' }}
            onClick={e => e.stopPropagation()}>
            <div className="p-4" style={{ borderBottom: '1px solid #1a1a22' }}>
              <h2 className="font-semibold flex items-center gap-2" style={{ color: '#B8A9C9' }}>📊 Feedback Analysis</h2>
            </div>
